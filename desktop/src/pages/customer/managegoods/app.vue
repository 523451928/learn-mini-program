  ...
  <div class="mp-container">
    <el-container>
      <el-aside width="138px" class="sup-el-aside">
        ...
        <left-menu defaultIndex="3" v-show="LeftMenuVisible" :permissions="permissions" ></left-menu>
      </el-aside>
  ...
  methods: {
    ...
    getMerchant() {
      this.$http.get('/api/User/merchant').then((res) => {
        ...
        let hasAllGoods = this.permissions.includes('Product')
        let hasGroupGoods = this.permissions.includes('ProductGroup')
        if (!hasAllGoods) this.$set(this.routeArr[0].children[0], 'disabled',  true) 
        if (!hasGroupGoods) this.$set(this.routeArr[0].children[1], 'disabled', true) 
      }).catch((err) => {
