<template>
  <div>
    ...
    <div class="submit-container">
      <el-button type="primary" class="mp-large-btn" @click="createPosition">保存</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createPosition() {
      ...
      this.ruleForm.actions = this.ruleForm.actions.concat(data.filter(i => i >= 0 || typeof i == 'string'))
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.$http.post('/api/user/permissions', this.ruleForm).then((res) => {
