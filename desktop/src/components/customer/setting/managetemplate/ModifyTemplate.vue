          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <el-input :disabled="ruleForm.type == 1 || !ruleForm.algorithms[0].schemes[0].isDefaultNationawide" type="number" min="0" max="100000000" v-model="ruleForm.algorithms[0].firstValue" @blur="validateNumber($event, ruleForm.algorithms[0].firstValue)"></el-input>
                </td>
                <td>
                ...
                <td>
                  <el-input :disabled="ruleForm.type == 1 || !ruleForm.algorithms[0].schemes[0].isDefaultNationawide" type="number" min="0" max="100000000" v-model="ruleForm.algorithms[0].continueValue" @blur="validateNumber($event)"></el-input>
                </td>
                <td>
...
export default {
  ...
  methods: {
    ...
    fillBack() {
      let templateId = this.$router.currentRoute.query.templateId
      if (templateId) {
        this.$http.get(`/api/product/freight/template/${templateId}`).then((res) => {
          this.ruleForm = res.data.data
          if (this.ruleForm.algorithms[0].schemes[0].province || this.ruleForm.algorithms[0].schemes[0].city || this.ruleForm.algorithms[0].schemes[0].area) {
            this.ruleForm.algorithms.unshift({
              "schemes": [
                {
                  "province": 0,
                  "city": 0,
          ...
          this.ruleForm.algorithms.forEach((item) => {
            item.regions = []
            item.schemes.forEach((itm) => {
              if (itm.area) {
                itm.areaCode = itm.area
                item.regions.push(itm.area)
              } else if (itm.city){
                itm.cityCode = itm.city
                item.regions.push(itm.city)
              } else if(itm.province) {
                itm.provinceCode = itm.province
                item.regions.push(itm.province)
              }
            })
          })
          this.appointAreaArr = this.ruleForm.algorithms.splice(1)
        }).catch((err) => {
    ...
    saveTemplate() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          // this.ruleForm.algorithms = []
          this.appointAreaArr.forEach((item) => {
          ...
          this.$http.put('/api/product/freight/template', ruleForm).then((res) => {
