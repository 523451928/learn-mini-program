...
Loadmore.prototype.init = function () {
  ...
  if (this.options.scrollLoad) {
    this.throttleFn = throttle(this.checkBottomReached.bind(this), 200)
    this.scrollEventTarget.addEventListener('scroll', this.throttleFn)
  }
}
...
Loadmore.prototype.appendBottomDom = function () {
  ...
  loadmoreBottomElm.innerHTML = this.options.bottomPullText
...
Loadmore.prototype.unbindEvent = function() {
  if (this.runEnvironment === 'mobile') {
    this.scrollEventTarget.removeEventListener('touchstart', this.startFn)
    this.scrollEventTarget.removeEventListener('touchmove', this.moveFn)
    this.scrollEventTarget.removeEventListener('touchend', this.endFn)
  } else {
    this.scrollEventTarget.removeEventListener('mousedown', this.startFn)
    this.scrollEventTarget.removeEventListener('mousemove', this.moveFn)
    this.scrollEventTarget.removeEventListener('mouseup', this.endFn)
  }
  if (this.options.scrollLoad) {
    this.scrollEventTarget.removeEventListener('scroll', this.throttleFn)
  }
}
...
Loadmore.prototype.fillContainer = function () {
  if (this.options.autoFill) {
    setTimeout(() => {
      ...
      console.log(this.containerFilled)
